...
if __name__ == "__main__":
    print("🧪 TEST RUNNER - EXEMPLE D'UTILISATION")
    
    # Exemple avec une classe de test factice
    class ExampleTestClass:
        def test_example_success(self):
            print("Ce test devrait réussir")
            return True
        
        def test_example_failure(self):
            print("Ce test devrait échouer")
            return False
        
        def test_example_with_exception(self):
            print("Ce test lève une exception")
            raise ValueError("Erreur de test")
    
    # Utilisation du TestRunner
    example_tests = ExampleTestClass()
    runner = TestRunner(example_tests)
    
    # Mode 1: Test unique
    print("\n1. 🎯 TEST UNIQUE:")
    runner.run_single_test("test_example_success")
    
    # Mode 2: Tests multiples
    print("\n2. 📋 TESTS MULTIPLES:")
    runner.run_multiple_tests(["test_example_success", "test_example_failure"])
    
    # Mode 3: Tous les tests
    print("\n3. 🚀 TOUS LES TESTS:")
